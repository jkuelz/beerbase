...
func main() {
	...
	router.POST("/addreview", func(c *gin.Context) {
			...
			result, err := db.Exec("INSERT INTO Review(id, ReviewerID, BeerID, Rating, Title, ReviewDescription, ReviewDate) VALUES((SELECT ISNULL(MAX(ID) + 1, 1) FROM Review), 1, 1, $1, $2, $3, (GETDATE()))", rating, title, description)
			if err != nil {
